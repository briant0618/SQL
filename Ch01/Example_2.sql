...
ORDER BY a.custid;
# 문제 24
SELECT `name`,`saleprice` FROM `Customer` AS a, `Orders` AS b
WHERE a.custid = b.custid; 
# 문제 25
SELECT `name`,SUM(`saleprice`) FROM `Customer` AS a, `Orders` AS b
WHERE a.custid = b.custid
GROUP BY `name`
ORDER BY `name`; 
# 문제26 
...
WHERE `price` = (SELECT MAX(`price`)FROM `Book`); # 이중 select 문
# 문제 29
